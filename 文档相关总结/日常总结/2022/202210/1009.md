...
success
 // //缓存里面数据库的总价计算
            ...
            //   if(lmap.has(val.id)) {
            //     let j = lmap.get(val.id)    //数据库中下标索引值
            //     foodsList[j].quantity += val.quantity
            //     val.number = 0;
            //     // if(val.number === 0) {pataData.splice(index,1)}
            //     // 更新本地存储
            //     wx.setStorage({
            //       data:pataData,
            //       key:'cartArray',
            ...
            //   }
            // })
            // // console.log(foodsList,'foodsList合并之后的数据数据');
            // // 删除缓存里number值为0的数组对象
            // pataData.forEach((val,index) => {
            //   if(val.number === 0) pataData.splice(index,1)
            //   wx.setStorage({
...
```
const lmap = new Map()
foodsList.forEach((val,index) => {
lmap.set(val.foodsId,index)
})
console.log(lmap);       //用map结构存数据库里的数据 用于之后合并数组
```
...
```
  // 添加到购物车
  // addToCart: function () {
  //   util.request(api.GoodsAddCart + '?userId=' + wx.getStorageSync('userId'), {
  //       goodsId: this.data.info.id,
  //       quantity: this.data.number,
  //       goodsProductId: this.data.guigeId
  //     }, "POST")
  //     .then(function (res) {
  //       // console.log(res);
  //       if (res.errno == 0) {
...
```
 wx.getStorage({
        ...
        success(res) {
          pataData = res.data
          // console.log(pataData,'取缓存取缓存取缓存');
          //缓存里面数据库的总价计算
          let newTotal = pataData.map(item => item.quantity);
          let price = pataData.map(item => item.quantity * item.price);
          let total = 0;
          ...
          // let productArray = pataData.concat(list)         //合并缓存数据和数据库里的数据

          console.log(pataData,'patapppp');
          console.log(list,'listllll');
          console.log(lmap,'lmap');
...
1101
一转眼十一月了 时间过的真快
今天拍了毕业照 挺丑的就是说 emm 到头来还是啥也不是
购物车 代码田炯了 本地测试暂时没问题 后期有问题再说吧
随时准备跑路
刚刚拉取好家园代码
怎么说捏
不是很想写 app
